...
fun main() {
    expect(0) {
        ...
        fun process(
            ...
            while (true) {
                ...
                for (task in currentTasks) {
                    ...
                    heightmap.getOrNull(task.first - 1)?.getOrNull(task.second)
                        ?.takeIf { heightCheck(it - currentHeight) }
                        ?.also { tasks += task.first - 1 to task.second }
                    heightmap.getOrNull(task.first + 1)?.getOrNull(task.second)
                        ?.takeIf { heightCheck(it - currentHeight) }
                        ?.also { tasks += task.first + 1 to task.second }
                    heightmap.getOrNull(task.first)?.getOrNull(task.second - 1)
                        ?.takeIf { heightCheck(it - currentHeight) }
        ...
        part1Result = process(startRow, startColumn, { it <= 1 }) { (r, c) ->
            r == endRow && c == endColumn
        }

        part2Result = process(endRow, endColumn, { it >= -1 }) { (r, c) ->
            heightmap[r][c] == 0
        }
    }
}